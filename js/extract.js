  ...
  img.onload = async function () {
    try {
      ...
      if (extractedData.startsWith("ENCRYPTED:")) {
        ...
        if (!password) {
          showNotification(
            "This watermark is encrypted! Please enter the password to decrypt it.",
            "error"
          );
          resetExtractButton();
          return;
        ...
        try {
          message = await WatermarkCrypto.decryptWatermark(
            encryptedHex,
            password,
            salt
          );
          if (!message) {
            showNotification(
              "Incorrect password! Could not decrypt watermark.",
              "error"
            );
            document.getElementById("extractedResult").classList.add("hidden");
            resetExtractButton();
          ...
          showNotification(
            "Error decrypting watermark. Incorrect password?",
            "error"
          );
          document.getElementById("extractedResult").classList.add("hidden");
          resetExtractButton();
        ...
        if (message && message.trim()) {
          ...
          showNotification(
            "Watermark extracted and decrypted successfully!",
            "success"
          );
        } else {
          document.getElementById("extractedResult").classList.add("hidden");
      ...
      showNotification(
        "Error extracting watermark. The image may not contain a valid watermark.",
        "error"
      );
      document.getElementById("extractedResult").classList.add("hidden");
    } finally {
