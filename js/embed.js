...
async function embedWatermark() {
    ...
    if (!watermarkText) {
        ...
        if (typeof showToast === 'function') showToast('Please enter watermark text!', 'error');
        return;
    }

    if (watermarkText.length > 1000) {
        showNotification('Watermark text is too long! Maximum 1000 characters.', 'error');
        if (typeof showToast === 'function') showToast('Watermark text is too long!', 'error');
        return;
    }
    ...
    try {
        ...
        const cryptoData = await WatermarkCrypto.encryptWatermark(watermarkText, password);
        ...
        img.onload = function() {
            try {
                ...
                const lengthBinary = messageToEmbed.length.toString(2).padStart(16, '0');
                const messageBinary = lengthBinary + messageToEmbed.split('').map(char => 
                    char.charCodeAt(0).toString(2).padStart(8, '0')
                ).join('');

                // Check if image is large enough to hold the message
                if (messageBinary.length > data.length / 4) {
                    showNotification('Image is too small! Please use a larger image or shorter text.', 'error');
                    resetEmbedButton();
                    return;
                }
                ...
                if (progressText) progressText.textContent = 'Generating watermarked image...';
                ...
                document.getElementById('watermarkedPlaceholder').classList.add('hidden');
                ...
                if (cryptoData.isEncrypted) {
                    showNotification('Watermark embedded successfully with password protection!', 'success');
                    if (typeof showToast === 'function') showToast('Watermark embedded with encryption!', 'success');
                } else {
                    showNotification('Watermark embedded successfully!', 'success');
                    if (typeof showToast === 'function') showToast('Watermark embedded successfully!', 'success');
                }
                ...
                showNotification('Error embedding watermark. Please try again.', 'error');
            } finally {
                resetEmbedButton();
            }
        };
